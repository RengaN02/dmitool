...
const UPNG = require("@pdf-lib/upng");
        ...
        for (const line of lines.slice(2)) {
            ...
            switch (key) {
                case "state":
                    ...
                    temporaryFrameCounts.push(0);
                    break;
                case "dirs":
                    state.dirs = parseInt(value);
                    break;
                case "frames":
                    temporaryFrameCounts[temporaryFrameCounts.length - 1] = parseInt(value);
                    break;
                case "delay":
                    state.delays = value.split(",").map(x => parseInt(x));
                    break;
                case "loop":
                    state.loop = parseInt(value);
                    break;
                case "rewind":
                    state.rewind = value === "1";
                    break;
                case "movement":
                    state.movement = value === "1";
                    break;
                case "hotspot":
                    const hotspot = value.split(",").map(x => parseInt(x));
                    if (state.hotspots === null) state.hotspots = [];
                    state.hotspots[hotspot[2] - 1] = [hotspot[0], hotspot[1]];
                    break;
            }
            if (
                temporaryFrameCounts[temporaryFrameCounts.length - 1] === state.dirs &&
                state !== undefined
            ) {
                dmi.states.push(state);
                temporaryFrameCounts.pop();
            }
        }

        const frameData = decoded.image.data.buffer;
        const frameWidth = decoded.image.width / temporaryFrameCounts.reduce((a, b) => a + b, 0);
        const frameHeight = decoded.image.height / temporaryFrameCounts.length;
        let frameIndex = 0;
        for (let i = 0; i < temporaryFrameCounts.length; i++) {
            const frameCount = temporaryFrameCounts[i];
            for (let j = 0; j < frameCount; j++) {
                const frameBuffer = frameData.slice(
                    frameIndex * frameWidth * frameHeight * 4,
                    (frameIndex + 1) * frameWidth * frameHeight * 4
                );
                const frame = new Image(frameWidth, frameHeight, new Uint8Array(frameBuffer), {
                    alpha: true
                });
                dmi.states[i].frames.push(frame);
                dmi.states[i].frames_encoded.push(frame.toDataURL());
                frameIndex++;
            }
        }
