...
async function start(args) {
    ...
    if(args[0] == "unpack") {
        ...
        Dmi.parse(filebuffer).then(dmi => {
            var bar = new ProgressBar('Creating Folder [:bar] :percent ', {
                complete: '=',
                incomplete: ' ',
                width: 20,
                total: dmi.states.length
            });
            var json = {
                width: dmi.width,
                height: dmi.height,
        ...
        var bar = new ProgressBar('Reading Folder [:bar] :percent ', {
            complete: '=',
            incomplete: ' ',
            width: 20,
            total: jsondata.states.length
        });
        new Promise((res, rej) => {
            ...
            console.log("The dmi file has started to be created, this may take some time for long files.")
        ...
        return console.log("pack or unpack")
    }
}
